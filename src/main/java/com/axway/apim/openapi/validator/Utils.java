...
public class Utils {
    ...
    public static String getContentStart(String someContent, int maxLength, boolean removeNewLines) {
        try {
            ...
            if (removeNewLines) {
                someContent = someContent.replaceAll("\\r?\\n", "");
            }
    ...
    /**
     * The content type may have been merged into http.headers by the policy
     * scripting filter, but additionally exists in http.content.headers. This
     * leads to a duplication of the Content-Type header in the backend request,
